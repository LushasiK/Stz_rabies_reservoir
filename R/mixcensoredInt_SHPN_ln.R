...
mixcensoredInt_SHPN_ln <- function (y1, y2, d=rep(1, length(y1)), wt=rep(1, length(y1)),
                                 dist="lognormal", n, cluster=NULL, classify="EM",
  ...
  while (iteration < maxiter) {
    ...
    if (classify=="EM"){
      for (i in 1:n) {
       ...
       parms[i,] <- c(cp$coefficients[1], exp(cp$coefficients[2]), cp$loglik)
